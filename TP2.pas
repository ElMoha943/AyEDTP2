...
procedure altaEmpresa();
	...
	begin
		...
		// repeat
		// 	ClrScr;
		// 	pass := true; //Hace que se repita el ingreso si el codigo es invalido.
  ...
  //             		readKey;
  //         end;
  //     if ((length(cod) > 3) or (length(cod) < 1)) then //Verifica el largo de lcodigo.
  //       begin
  //           pass := false;
  ...
  //       end;
	 ...
	 //  empresas[contEmpresas][0] := cod;
		...
		repeat
			...
			phone := empresas[contEmpresas][4];

			for i := 0 to length(phone) do
			pass := true;
        begin
        		writeln(phone[i]);
            if (phone[i] < '0') or (phone[i] > '9') then
            		pass := false;
        end;
      ClrScr;
		until (pass = true);
...
procedure altaCiudad();
	...
	begin
		...
		repeat //VALIDACIÓN
			duped := false;
			writeln(Utf8ToAnsi('Ingrese el código de la ciudad. (Verifique que no esté repetido)'));
			readln(cod);

			for i := 0 to contCiudades-1 do
				begin
					if cod = ciudades[i][0] then
						duped := true;
				end;

			if duped = false then
				cod := ciudades[contCiudades][0];
			...
			ClrScr;
		until (pass = true) and (duped = false);
		pass := false;
...
procedure altaProyecto();
	...
	begin
		...
		repeat //VALIDACIÓN
			duped := false;
			writeln(Utf8ToAnsi('Ingrese el código del proyecto. (Verifique que no esté repetido)'));
			readln(cod);

			for i := 0 to contProyectos-1 do
				begin
					if cod = proyectos[i][0] then
						duped := true;
				end;

			if duped = false then
				cod := proyectos[contProyectos][0];
		...
		for i := 0 to contCiudades-1 do
			begin
				if opt = IntToStr(i+1) then
					proyectos[contProyectos][4] := ciudades[i,0];
					ciudades[i][2] := IntToStr(StrToInt(ciudades[i][2]) + 1);
			end;
