	...
	begin
      ...
      for i := 0 to contEmpresas-1 do
          begin
              ...
              begin
                  pass := false;
              		writeln('Error: Ese codigo ya existe!');
              		readKey;
              		Break;
             	end;
          end;
      if ((length(cod) > 3) or (length(cod) < 1)) then //Verigica el largo de lcodigo.
        begin
            pass := false;
            writeln('Error: El codigo debe tener entre 1 y 3 caracteres.');
            readKey;
        end;
	  until (pass = true);
	  ...
	  ClrScr;
		...
		writeln('Empresa agregada exitosamente, pulse cualquier tecla para volver al menu anterior.');
		readKey;
...
procedure altaCliente();
	begin
		...
		writeln('Cliente agregado exitosamente, pulse cualquier tecla para volver al menu anterior.');
		readKey;
...
procedure altaCiudad();
	...
	begin
	  ...
	  ciudades[contCiudades][0] := cod;
		...
		writeln('Ciudad agregada exitosamente, pulse cualquier tecla para volver al menu anterior.');
...
procedure altaProyecto();
	...
	begin
      ...
      if ((length(cod) > 3) or (length(cod) < 1)) then //Verigica el largo de lcodigo.
        begin
            pass := false;
            writeln('El codigo debe tener un maximo de 3 caracteres.');
        end;
	  until (pass = true);
	  proyectos[i][contProyectos] := cod;
		...
		ClrScr;
		writeln('Proyecto agregado exitosamente, pulse cualquier tecla para volver al menu anterior.');
...
begin
	repeat
		ClrScr;
	    writeln('MENU EMPRESAS DESARROLLADORAS:'+#13+#10+'1. Alta de CIUDADES '+#13+#10+'2. Alta de EMPRESAS '+#13+#10+'3. Alta de PROYECTOS'+#13+#10+'4. Alta de PRODUCTOS (mantenimiento)'+#13+#10+'5. Mostrar Ciudades'+#13+#10+'0. Volver al menu principal');
	    repeat
	    	opt := readKey;
...
begin //Main
  ...
  repeat
  	ClrScr;
  	writeln('MENU PRINCIPAL: '+#13+#10+'1. EMPRESAS'+#13+#10+'2. CLIENTES'+#13+#10+'0. Salir'+#13+#10);
	   //menu principal
		repeat
      option := readKey;
